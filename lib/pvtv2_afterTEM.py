...
#inception+dilated. 俗称Texture Embedding Moudle (TEM). 四分支RFB，3,5,7的扩张率
...
class FeatureExtraction(nn.Module):
    def __init__(self, channel=32, pvt_pretrained=True, size_img=224):
        ...
        if self.pretrained:
            self.load_model('model_checkpoint/pvt_v2_b5.pth') 
...
class Network(nn.Module):
    ...
    def __init__(self, channel=32, pretrained=True, imgsize=224):
        super(Network, self).__init__()
        self.channel = channel
        self.imgsize = imgsize
        self.pretrained = pretrained
        self.feat_net = FeatureExtraction(self.channel)#transformer encoder
        self.decoder  = Decoder(self.channel)    
   
        # if self.pretrained:
            # self.load_model('./resnet50-pretrain.pth')
  
    def load_model(self,pretrained):
        pretrained_dict = torch.load(pretrained)#load_url(pretrained)
        
        print("Load pretrained parameters from {}".format(pretrained))
        self.feat_net.load_state_dict(pretrained_dict, strict=False)
    
    def forward(self, x, name=None, scene=None):
        fmap = self.feat_net(x, name, scene)
